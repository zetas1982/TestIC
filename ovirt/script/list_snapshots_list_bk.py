...
nombrevm = raw_input(" Nombre máquina a conocer Snapshot:")
...
# Describimos para que solo salga un snapshots
	...
	for disk in disks_service.list():
		...
		active = 1
        if (active) > 0  and vm_name == nombrevm: 
                print("Nombre Maquina:{vm},Snapshot_Descripcion:{snap},Snap_ID:{num}".format(
                        vm=vm_name,
